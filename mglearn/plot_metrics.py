...
def plot_confusion_matrix_illustration():
    ...
    plt.xticks([.25, .75], ["predicted 'not nine'",
               "predicted 'nine'"], size=20)
...
def plot_decision_threshold():
    ...
    fig, axes = plt.subplots(2, 3, figsize=(15, 8), subplot_kw={
                             'xticks': (), 'yticks': ()})
    ...
    axes[1, 1].plot(line, svc.decision_function(
        np.c_[line, 10 * np.ones(100)]), c='k')
    dec = svc.decision_function(np.c_[line, 10 * np.ones(100)])
    contour = (dec > 0).reshape(1, -1).repeat(10, axis=0)
    axes[1, 1].contourf(line, np.linspace(-1.5, 1.5, 10),
                        contour, alpha=0.4, cmap=cm)
    discrete_scatter(X_train[mask, 0], np.zeros(bla),
                     y_train[mask], ax=axes[1, 1])
    ...
    axes[1, 2].contourf(line, np.linspace(-1.5, 1.5, 10),
                        contour2, alpha=0.4, cmap=cm)
    discrete_scatter(X_train[mask, 0], np.zeros(
        bla), y_train[mask], alpha=.1, ax=axes[1, 2])
    axes[1, 2].plot(line, svc.decision_function(
        np.c_[line, 10 * np.ones(100)]), c='k')
