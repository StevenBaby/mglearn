...
def pca_faces(X_train, X_test):
    ...
    # Build NMF models with 10, 50, 100, 500 and 1500 components
    # this list will hold the back-transformd test-data
    reduced_images = []
    for n_components in [10, 50, 100, 500, 1500]:
        # build the NMF model
...
def plot_pca_faces(X_train, X_test, image_shape):
    ...
    for i, ax in enumerate(axes):
        # plot original image
        ax[0].imshow(X_test[i].reshape(image_shape), cmap=plt.cm.gray)
        # plot the four back-transformed images
        for a, X_test_back in zip(ax[1:], reduced_images):
            a.imshow(X_test_back[i].reshape(image_shape), cmap=plt.cm.gray)

    # label the top row
    axes[0, 0].set_title("original image")
    for ax, n_components in zip(axes[0, 1:], [10, 50, 100, 500, 1500]):
